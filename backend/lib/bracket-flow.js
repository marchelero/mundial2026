...
// r32: 16 -> r16: 8 (r32_1, r32_2 -> r16_1; r32_3, r32_4 -> r16_2)
// r16: 8 -> qf: 4
// qf: 4 -> sf: 2
// sf: 2 -> final: 1
...
function getNextMatch(round, position, isLoser = false) {
  if (round === 'sf' && isLoser) {
    return { round: 'third', position: 1, slot: 'home' };
  }
  if (round === 'final' || round === 'third' || round === 'sf' && !isLoser) {
    if (round === 'sf') return { round: 'final', position: 1, slot: position === 1 ? 'home' : 'away' };
    return null;
  }
  if (round === 'r32') {
    const nextPos = Math.ceil(position / 2);
    return { round: 'r16', position: nextPos, slot: position % 2 === 1 ? 'home' : 'away' };
  }
  if (round === 'r16') {
    const nextPos = Math.ceil(position / 2);
